...
class UkhsaApi {
    ...
    async getCovid19AdmissionByDay(pagination?: Pagination) {
        const { data } = await this.#getCovid19Metric(
            Metric.COVID_19_HEALTHCARE_ADMISSION_BY_DAY,
            pagination,
        )
        return data ?? null
    }
    ...
    async getCovid19DeathsByDay(pagination?: Pagination) {
        const { data } = await this.#getCovid19Metric(
            Metric.COVID_19_DEATHS_ONS_BY_DAY,
            pagination,
        )
        return data ?? null
    }
    ...
    getAllCovid19AdmissionByDay() {
        return this.#getAllCovid19Metric((x) =>
            this.getCovid19AdmissionByDay(x),
        )
    }
...
const ukhsaApi = new UkhsaApi()

export default ukhsaApi
